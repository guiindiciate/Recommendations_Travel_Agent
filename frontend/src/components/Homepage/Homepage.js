...
const Homepage = (props) => {
  ...
  const imgData = [{
    ...
    gender: 'female',
    data: [
    { id: 1, src: "/imagerepo/image18.png", name: "Footwear" }, //need to change
  ...
  const fetchRecommendation = async (currentData) => {
    ...
    try {
      ...
      if (response?.response) {
        const { travel_recc, images } = response.response;
        obj.travel_recc = travel_recc;
        // `images` comes directly from the backend style tool (remote URLs)
        obj.styling_images = Array.isArray(images) ? images : [];
        console.log("obj -> ", obj)
        setEventDetails(userDetails.user_events);
