...
const UpcomingEvents = ({
  ...
  const [imageData, setImageData] = useState<WardrobeImage[]>([]);

  const [open, setOpen] = useState(false);
  const [stylingImages, setStylingImages] = useState<StylingImage[]>([]);
  const [currentFact, setCurrentFact] = useState(facts[0]);

  // Clear styling images when a new AI request is in-flight to avoid showing stale results.
  useEffect(() => {
    if (isLoading) {
      setStylingImages([]);
    }
  ...
  useEffect(() => {
    fetch("/json files/imgmap.json")
      .then((res) => res.json())
      .then((data: WardrobeImage[]) => {
        if (!Array.isArray(data)) {
          console.error("Error: imgmap.json data is not an array!");
          return;
        }

        const wardrobeIds = (wardrobeDetails || []).map(String);
        const filteredImages = data.filter((img) =>
          wardrobeIds.includes(img.id.trim())
        );
        setImageData(filteredImages);
        console.log("Image Data ->", filteredImages, currentEvt);

        const stylingPayload =
          currentEvt?.styling_images ??
          currentEvt?.response?.images ??
          currentEvt?.response?.image_ids ??
          [];

        // Remote images coming from backend (SerpAPI) already have URLs
        if (
          Array.isArray(stylingPayload) &&
          stylingPayload.length > 0 &&
          typeof stylingPayload[0] === "object" &&
          (stylingPayload[0].image_url || stylingPayload[0].thumbnail)
        ) {
          const filteredRemote = stylingPayload.filter((img: any) => {
            const src = `${img?.source || ""}`.toLowerCase();
            const link = `${img?.link || ""}`.toLowerCase();
            return !src.includes("tiktok") && !link.includes("tiktok");
          });
          setStylingImages(filteredRemote);
          return;
        }

        let stylingIds: string[] = [];
        if (typeof stylingPayload === "string") {
          try {
            const parsed = JSON.parse(stylingPayload.replace(/'/g, '"'));
            if (Array.isArray(parsed)) {
              stylingIds = parsed.map(String);
            }
          } catch (error) {
            console.error("Error parsing styling_images:", error);
          }
        } else if (Array.isArray(stylingPayload)) {
          stylingIds = stylingPayload.map(String);
        }

        if (stylingIds.length) {
          const filteredStylingImages = data
            .filter((img) => stylingIds.includes(img.id.trim()))
            .map((img) => ({
              id: img.id,
              description: img.description,
              image_url: `/${img.imgfilepath}`,
              imgfilepath: img.imgfilepath,
              source: "wardrobe",
            }));
                        ...
                        <Row narrow>
                          {stylingImages.map((img) => {
                            const imageSrc =
                              img.image_url ||
                              img.thumbnail ||
                              (img.imgfilepath ? `/${img.imgfilepath}` : "");
                            const title = img.description;
                            return (
                              <Column key={img.id || img.link || imageSrc} lg={4}>
                                <Tile
                                  style={{
